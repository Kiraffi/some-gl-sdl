...
pub unsafe fn load_gl_func<T>(name: &str)  -> Option<T>
{
    match &std::ffi::CString::new(name)
    {
        ...
        Err(_) =>
        {
            assert!(true, "Load GL func {} failed.", name);
            return None;
        }
    }
}
...
pub unsafe fn glGetString(name: GLenum) -> *const GLubyte {  funcs::glGetString.unwrap()(name) }
...
macro_rules! gl_loader 
{
    (
        ...
        pub fn $fn( $($arg: $t),* ) -> $res
        {
            println!("You called {:?}", stringify!($fn));

            $(
                println!("   with param: {:?}, type: {:?}, value: {:?}", stringify!($arg), stringify!($t), $arg);
            )*

            println!("returns {:?}", stringify!($res));
        } )*
    }
}

gl_loader!(
    //fn glGetStringi(name: i32, index: u32) -> (),
    fn poo(name: u32) -> ()
);
